...
print("planet data rows - ",planet_data_rows[0])
...
print("length of planet data rows - ",len(planet_data_rows))
...
print("length of hd_10180 -",len(hd_10180_planets))
print("hd_10180",hd_10180_planets)
...
gravity = planet_gravity*4
fig = px.scatter(x=planet_radiuses, y=planet_masses, size=planet_gravity, hover_data=[planet_names],)
fig.show()

low_gravity_planets = []
for index, gravity in enumerate(planet_gravity):
  if gravity > 10 and gravity < 90:
    low_gravity_planets.append(planet_data_rows[index])

print("length of low gravity planets",len(low_gravity_planets))

planet_types_values = []
for type in planet_data_rows:
    planet_types_values.append(type[6])

print(list(set(planet_types_values)))
terrestial_planets = []
super_earth_planets = []
for data in planet_types_values:
    if data == 'Terrestrial':
        terrestial_planets.append(data)
    elif data == "Super Earth":
        super_earth_planets.append(data)

print("length of terrestial planets",len(terrestial_planets))
print("length of super earth planets",len(super_earth_planets))
...
print("length of suitable planets",len(suitable_planets))
...
print("length of suitable planets after filtering out orbital radius and period",len(suitable_planets))
print("lenth of Planets in a range of 0.38 - 2 AU",len(goldilock_planets))
...
print("length of planets with optimal speeds",len(speed_supporting_planets))
